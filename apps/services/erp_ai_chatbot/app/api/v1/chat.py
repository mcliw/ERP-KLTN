...
def _build_context_prefix(context_json: dict | None, recent_pairs: list[dict], max_chars: int = 2500) -> str:
    payload = {
...
def _extract_context_from_result(result: dict) -> dict:
    ...
    return {
        "focus_ref": focus_ref,
        "focus_payload": safe_focus,  # ✅ đã lọc
...
def chat(req: ChatRequest):
    ...
    try:
        ...
        recent_msgs = fetch_recent_messages(db, conversation_id, limit=20) if conversation_id else []
        ...
        # ===== 4) Nếu chưa đăng nhập: CHỈ cho phép rag_policy (public) =====
        if not auth_ctx.is_authenticated or not auth_ctx.role:
            det = detect_module_llm(message=enriched_message, role=req.role)
            if det.get("selected_module") == "rag_policy" and not det.get("needs_clarification", False):
                ...
                # ===== Save history (public cũng lưu được nếu có user_id) =====
                if conversation_id:
                    append_message(
                        db,
