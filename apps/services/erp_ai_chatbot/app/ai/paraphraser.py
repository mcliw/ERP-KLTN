...
def _safe_enough(paraphrased: str, allowed: str, expected_codes: set[str] | None = None) -> bool:
    ...
    # 6) Chặn bịa “nghiệp vụ nhạy cảm” nếu answer gốc + facts không có
    sensitive = ["trả lại", "hoàn trả", "hủy", "huỷ", "refund", "return", "canceled", "cancelled"]
    if _contains_any(paraphrased, sensitive) and not _contains_any(allowed, sensitive):
        return False
