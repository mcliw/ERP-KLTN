...
def compose_answer_with_llm(module: str, user_question: str, step_infos: list[dict]) -> str:
        ...
        "tool_results": [
            {
                "step_id": si.get("id"),
                "tool": si.get("tool"),
                "ok": (si.get("result") or {}).get("ok") if isinstance(si.get("result"), dict) else None,
                "thong_diep": (si.get("result") or {}).get("thong_diep") if isinstance(si.get("result"), dict) else None,
                "data": _clip_data((si.get("result") or {}).get("data") if isinstance(si.get("result"), dict) else si.get("result")),
            }
    ...
    sys = """
Bạn là trợ lý ERP - module Supply Chain.
Trả lời NGẮN GỌN, đúng trọng tâm theo câu hỏi.
QUY TẮC:
- Chỉ dùng dữ liệu trong tool_results. Không tự suy đoán ngoài dữ liệu.
- Nếu tool_results là list rỗng/không có dữ liệu phù hợp: trả "Không có dữ liệu phù hợp."
- Nếu câu hỏi hỏi 1 ý: chỉ trả 1 ý đó (không thêm info thừa).
- Ưu tiên 1-2 câu. Nếu cần liệt kê: tối đa 5 dòng.
"""
